...
template <
	typename _Key,
	typename _Value,
	typename _Key_Container = std::set<_Key>,
	typename _Mapped_Container = std::set<_Value>>
class bimap
{
public:
	class const_iterator : public std::iterator<std::bidirectional_iterator_tag, std::pair<const _Key&, const _Value&>>
	{
	public:
		typedef const_iterator self_type;
		typedef std::pair<typename _Key_Container::iterator, typename _Mapped_Container> value_type;
	...
	using rkey_type = _Value;
	using rvalue_type = _Key;
private:
...
private:
	static_assert(
		std::is_same_v<key_type, typename _Tree::key_type>, "bimap::key_type has to match _Container_Type::key_type!");
	static_assert(std::is_same_v<rkey_type, typename _RTree::key_type>, "bimap::rkey_type has to match _Reverse_Container_Type::key_type!");
private:
	_Tree m_key_tree;
	_RTree m_value_tree;
private:
	inline std::size_t index_of_key(const key_type& key) const
	{
...
using unordered_bimap = bimap<_Key, _Value, std::unordered_set<_Key>, std::unordered_set<_Value>>;